...
def fixFirstClash(clashes, shapes = 0):
    """takes in list of base names that are clashing (ie. "pSphere1", meaning that there are multiple instances of this name), 
    if there's more than one clash in the list, redo the clash detection to make sure names stay relevant"""
...
def clash(fixClashes = 1):
    """launch the clash detecting/fixing. Only argument is whether to fix the clashes or not (1/0)
        returns a bool to tell whether there is a clash or not
    """

    clash = detectClashes(fixClashes)
    if fixClashes == 1: #if we are fixing things, do one more pass to make sure
        clash = detectClashes(fixClashes)
    
    shClash = detectShapeClashes(fixClashes)
